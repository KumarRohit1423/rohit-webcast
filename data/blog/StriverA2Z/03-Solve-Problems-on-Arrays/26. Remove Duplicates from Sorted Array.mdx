...
Given an integer array `nums` sorted in **non-decreasing order** , remove the duplicates [**in-place**](https://en.wikipedia.org/wiki/In-place_algorithm" target="\_blank) such that each unique element appears only **once** . The **relative order** of the elements should be kept the **same** . Then return the number of unique elements in `nums`.
...
class Solution {
 public:
  int removeDuplicates(vector<int>& nums) {
    int filledIDX = 0, currVal;
    for (int i = 0; i < (nums.size() - 1); i++) {
      if (nums[i] != nums[i + 1]) {
        nums[++filledIDX] = nums[i + 1];
      }
    }
    return (filledIDX + 1);
  }
